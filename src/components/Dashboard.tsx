...
export default function Dashboard() {
  ...
  async function fetchDashboardData() {
    setLoading(true);
    try {
      // 1. Fetching from 'products' table using your specific columns
      ...
      if (products && products.length > 0) {
        ...
        products.forEach(item => {
          // MAPPING TO YOUR SCHEMA: current_stock, cost_price, reorder_level
          const qty = Number(item.current_stock || 0);
          const price = Number(item.selling_price || 0);
          const min = Number(item.reorder_level || 0);
          const category = item.unit || 'General'; // Using 'unit' as a category proxy if category isn't there
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              {categories.map((cat, idx) => (
                <div key={idx}>
                  ...
                  <div className="w-full bg-slate-100 rounded-full h-4 overflow-hidden">
                    <div 
                      className="bg-amber-500 h-full rounded-full transition-all duration-1000 shadow-[0_0_10px_rgba(245,158,11,0.2)]"
          ...
          <div className="bg-slate-900 p-8 rounded-[2.5rem] text-white shadow-2xl flex flex-col justify-between relative overflow-hidden">
             <div className="relative z-10">
              ...
              <div className="p-6 bg-white/5 rounded-3xl border border-white/10 mb-6">
                <p className={`text-4xl font-black italic ${stats.lowStock > 0 ? 'text-red-500' : 'text-amber-500'}`}>
                  {stats.lowStock > 0 ? 'ACTION' : 'GOOD'}
                </p>
                <p className="text-slate-400 font-bold text-[10px] uppercase mt-2">
                  {stats.lowStock > 0 ? `${stats.lowStock} items need reorder` : 'Inventory levels stable'}
                </p>
              </div>
            </div>
            <button className="relative z-10 w-full py-4 bg-amber-500 text-slate-900 font-black rounded-2xl hover:bg-amber-400 transition-all uppercase text-xs tracking-widest">
              Update Products
            </button>
          </div>
        </div>
